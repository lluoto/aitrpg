...
> 生成于 2026-08-23 05:03  ·  刷新：`bun scripts/handoff.ts`
> 状态快照看 `docs/now.md`；这份讲的是**怎么接手**。

## 这是什么

`C:\aitrpg\poc` —— CoC 7e 跑团引擎。核心是「模组数据 + 规则引擎 + LLM 叙事」
跑完一局《普瑞米尔的谷仓》。**当前 HEAD**：d976680 fix: every knowledge reveal ended with "我知道的就这些了", including the ones that didn't  ·  **测试**：1766 条 / 88 文件（1 失败）（基线 1766，一致）
