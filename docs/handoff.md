...
> 生成于 2026-08-23 11:00  ·  刷新：`bun scripts/handoff.ts`
> 状态快照看 `docs/now.md`；这份讲的是**怎么接手**。

## 这是什么

`C:\aitrpg\poc` —— CoC 7e 跑团引擎。核心是「模组数据 + 规则引擎 + LLM 叙事」
跑完一局《普瑞米尔的谷仓》。**当前 HEAD**：c90cbef chore: turn on the unused-symbol checks, then read what fell out  ·  **测试**：1802 条 / 92 文件，全绿（基线 1802，一致）
