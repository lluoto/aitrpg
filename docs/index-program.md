...
| tools/_run-ingest.ts | **摄取管线的端到端实跑器**（约 250 行）：读 PDF → 清洗 → 切分 → **块分类 → 建场景骨架 → 条目分类 → 建 ModuleItem** → 对基准 diff。含 `RecordingClient`，把**每一次** LLM 调用的 prompt 与原始响应都落盘（两次调用，单组字段会让后一次覆盖前一次，而那是「模型到底答没答」的唯一证据）。下面 §模组摄取 里所有实跑数**全部出自它**——`身份覆盖 20/20`、`物品覆盖 9/10`、`精确率 9/17`、`差异 178 处`，以及上一轮的 `命中 20 / 误报 7 / 漏报 0`。重跑它是唯一能复现这些数的途径。它还印三样光看 diff 看不出来的：口径对账（覆盖+误报 是否等于生成数，防重名两边同时吃掉）、悬空引用（`item.sceneId` 是否都指得到人）、以及逐条目的分类结果 |
...
| 条目分类（`▶` 是什么） | `src/ingest/classify-items.ts` | **已完成**，21 测试。实跑 37 送分类 / **返回 37**，分布 item 13 / clue 12 / event 5 / trap 4 / connection 3 |
| ModuleItem 抽取 | `src/ingest/build-items.ts` | **已完成**，22 测试。实跑 **基准 10 个物品覆盖 9**（上限 9）、生成 17 个、**精确率 9/17**、误报 7 |

> ingest 测试合计 **219**（上表逐文件相加）。这个数是逐文件 `bun test` 实测的，不是估的——
> 本轮就有两处表内数字写错过（`build-scenes` 写成 16 实为 14，`build-items` 写成 20 实为 22），
> 是最终评审对着 `bun test` 逐文件核出来的。改这张表时请照着实测值写。
...
| 条目分类分布 | item 13 / clue 12 / event 5 / trap 4 / connection 3 |
| 建成物品 | 17 个，`provenance` **9 条**（第一次有代码往 `Provenance[]` 里写东西。注意它**没有**放进被 diff 的那份 `ModuleData`——生成留痕不是模组内容，基准永远不会有，塞进去只会凭空多十几条 `extra`。它单独落 `provenance.json`） |
| 基准物品覆盖 | **9 / 10**（上限 9） |
| 精确率 | **9 / 17** |
| 误报 | 7 个，**全部是基准里的 `Clue`**，一个凭空的都没有 |
| 17 个怎么分的 | 9 真阳 + 7 误报 + **1 个重复**（`驾驶证`，PDF 里写了两次而基准只收一个）。这一项必须单列：第一次实跑正是因为把它算进真阳，报出了 `10/10`——比理论上限还好 |
...
### 根因：PDF 读取顺序把整行甩出了它该在的条目（待修）

`证物室` 的 `▶防盗门的钥匙` 正文在生成侧截断成「…这种先进防盗门可」，
而基准是「…这种先进防盗门可不多见。」——缺掉的「不多见。」**单独成行落在了场景正文里**。
同一个原因把「如果伤害大于调查员耐久最大值的一半，则有截肢的风险。」甩出了 `捕兽夹` 条目，
**这才是 `maimAtHpRatio` 没抽到的真原因，不是抽取器弱**。

四个陷阱与基准逐字段比，8 处分歧里：3 处基准对（`maimAtHpRatio` / `ongoing.until` / `firstAid`，
**全都因为那条机制写在场景正文而不在 `▶` 条目里**），3 处生成物对
（音响陷阱的 `sanCost`、硫酸陷阱的 `avoid: 闪避`、以及正确地没编造 `immuneNarration`），
2 处是尚未实现的线索交叉引用。**其中 2 处「生成物更对」还被报告记成了 `extra`**——
按既定立场，该改的是基准。

修这个甩行问题的收益比继续加字段大：它一个人造成了一处字段损坏、一处真实内容丢失，
且位于所有下游之上游。
