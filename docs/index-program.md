...
| tools/_exp-clue-followup.ts | **实验脚本**：换一根轴对 item 族追问一次，拿评分键算前后对比。产物 `exp-clue-followup.txt`。它先否掉了「得到东西 vs 知道事情」（−2），再验成了「带走的东西 vs 去翻的地方」（**+7，0 弄坏，3 次稳定**）。**验一个分类修法的标准做法就是它**——不改管线、一次调用、有前后数 |
...
### 修法验证成功：换一根轴追问一次，67% → 88%（2026-08-20，跑了 3 次）
...
- 3 条 `event` 族（`p5:L15`、`p5:L17`、`p13:L11`）——这次没追问它们。
  它们的形态是「条件叙述，但它就是那条线索」，需要另一根轴。
- 1 条 `p8:L6`「抽屉里的…转购协议」——那条 `▶` 行**正文为空**，只有名字，模型没东西可判。
  这不是分类问题，是根因 B（同页续句）的下游表现。

**要落地成管线的话**：这是一个两段式分类，第二段只对 `item` 族触发（实测 15 条），
多一次 LLM 调用。落地前要注意的是本文件 §分类器的逐条输出不是独立的 那一条——
追问也是整批发的，同样会有批内耦合。
