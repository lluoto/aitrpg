...
### ❓ 待定：4 条 core 线索的技能检定永远不会执行（2026-08-20，未改）

`runClueCheck` 遍历 `findMethods`，碰到第一个 observation/automatic/item
...
后三条的技能描述明显是**比观察更深一层的动作**，而引擎把完整 `revelation` 在观察时就白送了。
...
Good End 要 `!老旧文件`，永远不可能成立。实测随机玩法 10 局全是 True End，
其中一局只走了 11 个场景、**一次调查都没做**。

**这不是我改出来的**：拿 failback 修复之前的版本跑同样 10 局，
9 局 True End + 1 局 Normal End（那 1 局正是报废没走完的）。

**为什么没自行改**：数据模型一条线索只有一个 `revelation`，
表达不了"观察看见一半、检定看见全部"。要修得在
①拆成两条线索（改数据、动作者原意）②让 skill 优先、passive 退化成兜底（改难度）
之间选，那是设计取舍不是 bug 修复。
