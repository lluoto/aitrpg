...
| tools/_diag-p15.ts | 取证脚本：列出页内以 ASCII 连续句点收尾的行及其上下文形态，并倒出 p15 整页原始行。产物 `tools/ingest-out/diag-p15.txt`。**它把「ASCII 省略号有两种含义」这个症状级诊断推进到了真因**——p15 的结局与奖励表整片被焊（见 §p15 那节）。做结局那一轮把它扩到全书，先量形态判据的误判率再动 `shouldJoin` |
...
**修之前要证明的事**（这一节没写修法，是因为这两条还没量）：
纯拉丁行、以数值收尾的行，这两类形态判据在**其余 17 页**上的误判率是多少。
本轮已经两次被未测量的规则咬到（同页贪心吸收四成误吃、页边界省略号 6 处错 1 处），
所以不带这个数字就不要动 `shouldJoin` —— 它的页内语义是全书 255 行的地基。
取证脚本 `tools/_diag-p15.ts` 已经能列出候选行及其上下文形态，扩到全书即可。
