...
「…这种先进防坍门可」截断就在里面），且改动局限在页边界的续接判断上。
...
| 条目 | `sourceRef` 写的 | PDF 实际 | 差 |
|---|---|---|---|
| `捕兽夹` | p9:L13 | p9:L40 | 27 行 |
| `防盗门的钥匙` | p6:L17 | p6:L67 | **50 行** |
| `中控台的开关` | p11:L10 | p11:L58 | 48 行 |
| `硫酸陷阱` | p10:L4 | p10:L12 | 8 行 |

而 `Provenance.sourceRef` 的文档原本写的是「PDF 页号 + 页内行号」——
**照它去 PDF 第 6 页数到第 17 行，落点跟 `防盗门的钥匙` 差 50 行**。
