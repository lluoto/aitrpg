...
修法方向（未验证，下一轮的活）：A 要么把行连接改成跨页做（`cleanPageText` 收整本而非单页，
接口要变），要么在 `sectionize` 里补一道跨页首行的续接判断；B 是 `sectionize` 的条目
从「取一行」改成「取到下一个 `▶`/标题为止」。B 更小、收益更直接，且能独立验证：
四个陷阱里 `maimAtHpRatio` 与 `firstAid` 两处「基准对」应当随之消失。
...
修根因 B 的收益比继续加字段大：它位于所有下游之上游，一处修好同时消掉一处字段截断、
一处真实内容丢失、以及两处「基准对」的陷阱分歧。而且它可独立验证——改完重跑，
`maimAtHpRatio` 与 `firstAid` 应当从「基准对」里消失。
