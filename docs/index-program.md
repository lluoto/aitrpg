...
| tools/_diag-joined.ts | 取证脚本：`joinPages` 到底接了哪几处、哪些条目正文因此变了。产物 `tools/ingest-out/diag-joined.txt`。**它是「分类器逐条输出不独立」那条结论的证据来源**——正文没变的条目标签照样会翻，所以不能按单条归因 |
...
| 文本清洗 | `src/ingest/clean-text.ts` | **已完成**，32 测试。`cleanPageText` 管页内、`joinPages` 管跨页（实测接了 6 处），两者共用同一个 `shouldJoin` 判据 |
...
实际接了 **6 处**页边界（p1→p2、p4→p5、p5→p6、p6→p7、p10→p11、p17→p18）。
比 `_diag-absorb` 报的「跨页 4 行」多两处，因为那个只数「跟在 `▶` 条目后面」的跨页续行，
`joinPages` 连普通正文的跨页也接——两个分母，不是矛盾。
