...
| tools/_exp-conn-infer.ts | **实验脚本**：让 LLM 看着场景清单推连接图，用配对判据打分。产物 `exp-conn-infer.txt` 与原始响应。结果 **F1 0.77 / 正确边可达 9/20**——边很准但通往农场的三条路全缺，模组断成两半。**判据第一次实用就抓到了 F1 看不出的问题** |
...
**下一轮该试的**（都没验）：把「必须连通」写进 prompt（告诉模型每个场景都要能从起点走到）；
或者产出后做一次连通性修补——找出不连通的分量，只补跨分量的边，这样不伤精确率；
或者两段式：先让模型给出场景的从属层级，再由层级机械导出边。
