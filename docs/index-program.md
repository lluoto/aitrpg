...
> ⚠️ **补注（2026-08-20）：这个 94% 至今没进管线。**
> `tools/_run-ingest.ts` 里一处 followup 都没有，实跑 report 报的仍是 22/33（67%）。
> 下面这一节讲的是修法**验证成功**，不是修法**已经生效**。
...
⚠️ **31/33 是实验脚本的成绩，不是管线的成绩**（2026-08-20 补注）。
`tools/_run-ingest.ts` 里**一处 followup 都没有** —— 分族追问只活在
`tools/_exp-clue-followup.ts` 里，从没接进管线。
实跑 report 现在报的仍是 **22/33（67%）**，那才是管线的真实水平。
物品覆盖 9/10 是管线的真数，条目分类 31/33 不是，两者别并列着读。
（同样的坑在 connections 那边也踩过一次：F1 0.72 也是脚本的成绩。
**这条线上「实验成绩」与「管线成绩」已经分叉了，看数先问是哪一种。**）
