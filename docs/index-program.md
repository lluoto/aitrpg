...
| 场景骨架 | `src/ingest/build-scenes.ts` | **已完成**，13 测试。实跑 **基准 20 个场景按 name 命中 17** |
...
**按内容算 20/20，按 name 严格配算 17/20** —— 表里记的是后者，因为它是机器能自动判的那个。
...
所以这 7 个不是模型乱认，是**基准把一部分可调查地点收成了线索**——校准是双向的，下一轮该决定的是收哪边。
