...
| 场景骨架 | `src/ingest/build-scenes.ts` | **已完成**，16 测试。实跑 **严格 17 + 名字变体 3 = 身份覆盖 20/20，真漏报 0，真误报 7**（**与上面那行不同口径**：name 严格配。单次采样、`temperature: 0.1`，非确定性——别当回归基线，详见下） |
| 条目分类（`▶` 是什么） | `src/ingest/classify-items.ts` | **已完成**，21 测试。实跑 37 送分类 / **返回 37**，分布 item 13 / clue 12 / event 5 / trap 4 / connection 3 |
| ModuleItem 抽取 | `src/ingest/build-items.ts` | **已完成**，20 测试。实跑 **基准 10 个物品覆盖 9**（上限 9）、生成 17 个、**精确率 9/17**、误报 7 |
...
### 实跑数（2026-08-19，单次采样 `temperature: 0.1`）

| | 数 |
|---|---|
| 条目送分类 / 返回 | 37 / **37**（全文 39 个 `▶`，2 个在 npc 块上不进） |
| 条目分类分布 | item 13 / clue 12 / event 5 / trap 4 / connection 3 |
| 建成物品 | 17 个，`provenance` **9 条**（`ModuleData.provenance` 第一次有内容） |
| 基准物品覆盖 | **9 / 10**（上限 9） |
| 精确率 | **9 / 17** |
