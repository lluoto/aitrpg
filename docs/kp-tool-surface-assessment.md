...
4. **`game-session.ts` 已 2920 行**（2026-08-29 复核：实测已涨到 3729 行，本条数字是初稿写下时的快照，未跟着后续改动同步——就地更正，不追溯每次涨了多少）。25 个 handler 与 9 个 mutator 同处一文件，是上述三点的物理成因。`apply_action` 收敛与该文件拆分是同一件事。
...
- 顺带把 `game-session.ts` 中相关 handler 按职责迁出，缓解行数问题（2026-08-29 复核：3729 行，仍在涨）。
...
## 六点五、已建成但从未接线的子系统（4 项里 3 项现已接线）

排查闸门迁移时顺带发现的。这些不是缺陷，是**未完成的接线**——代码、数据、测试都在，只是没有任何生产路径调用它们。记在这里是为了避免两种误判：以为它们在工作，或者以为它们不存在而重新造一遍。**标题保留原文，因为下面 4 项里已经有 3 项标了「已接线」——历史标题描述的是排查当时的状态，不是现在。**
...
> 此条最初写作「从未被调用」，是错的。`index.ts:974` 与 `play-module.ts:89` 一直在调 `createCoCCharacter`。没接上的只是**前端连的 HTTP 路径**：CLI 与 play-module 用的是正确的 CoC 建卡器，`game-session.ts` 的三个建卡点却都走通用的 `CharacterFactory.generate()`。同一个系统有两套建卡，比整个系统没人用更难发现。
