 ...
 *
 * ⚠ 这里原先写着"提示词第 7 条主动要求 NPC 台词带括号神态"——那是
 * `41a8e66`（2026-08-18）反转之前的旧文本。**现在的第 7 条是反过来的**：
 * 默认直接开口说话，括号神态不是每句都要，只在确有叙事价值时才写，且
 * 优先穿插在台词中间或末尾；明确禁止把括号描写当成固定开头
 * （`src/llm/npc-dialogue-prompts.ts:91`）。
 *
 * 但这不等于函数该删：提示词不再"要求"括号，不等于 LLM 不会"产出"
 * 括号——第 7 条仍然允许穿插句中，且 voice-readiness.md 第五节记着
 * 括号神态是比单一 mood 标签更精细的韵律提示（`（声音低沉）` 对应降低
 * 音高、放慢语速）。只要提示词还允许它出现，送进 TTS 前就还得切分，
 * 不能直接照着念出来（「抬眼打量他们」）。所以这里依然是切分，不是
 * 剥离——这一点没变，变的只是"括号从必需变成可选"。
 *
