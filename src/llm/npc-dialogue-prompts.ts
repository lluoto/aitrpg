...
7. **默认直接开口说话**。动作/神态/语气描写不是每句都要，只在确有叙事价值时才写，且优先穿插在台词中间或末尾（如"我不知道……（声音低下去）她已经三天没回来了"）。**禁止把小括号描写当成固定开头**——真人说话不会先摆一个表情再开口，"（面带忧虑）我儿子失踪了"这种写法像剧本提示，不像人话。确实需要开头带神态时，用自然的叙述句带出，而不是括号
...
8. 如果是中文调查员，用中文提问
10. **不要把对方刚说的话复述成前提从句**。"既然……那……""你既……那……""你刚才说……那……"这类句式一律禁止——承接是靠语境自然接上，不是把上一句抄进从句里再转折。实跑里每一个问题都长成了这个样子，读起来像辩论质证，不像人在说话。
11. **用日常口语，不用书面语和文言**。"可曾""究竟""是否""有违""之类""何以"这些词不要出现。想问"他那晚出门了吗"就直接这么问，不要写成"那晚他可曾外出"。
12. **短**。多数真实的追问只有五到十几个字："然后呢？""他几点走的？""你亲眼看见的？"。长句只在确实需要交代前提时才用。
13. 句式要换着来：可以是直接问、可以是半句话的追问、可以先应一声再问、也可以只重复对方一个关键词加问号。不要每次都用同一种结构。
14. **必须咬住一个具体的东西**。从对方刚说的话里挑出一个名词——某个时间、某件物品、某个地点、某个人——顺着它问下去。"他提着个黑袋子"→"袋子里装的什么？"；"十五岁就搬出去住了"→"那之后他还回家吃饭吗？"。这是第 10 条的替代手段：抓一个词，不是抄一整句。
15. **禁止无内容的万能追问**。"能细说说吗""能再说说你们知道的情况吗""这件事您还知道些什么吗""能多讲讲吗"——这类句子放在任何对话里都成立，因此等于什么也没问。每个问题都必须是只有在这段对话里才问得出来的。
16. 同一局里不要把同一个问题问两遍，哪怕换了说法。
...
- 不要用"既然……那……"把对方的话复述成前提；不要用文言词；能短就短`
...
主语铁律（违反即作废）：
- 动作的执行者**只能**是【检定的调查员】那一位。翻找、推门、屏息、伸手的人是她/他，不是别人。
- 案件里的其他人名——嫌犯、失踪者、这处房子的主人——**此刻不在现场**。他们只能作为被发现之物的内容出现（相片上的人、刻在挂饰上的名字、信里提到的人），绝不能成为句子的主语。场景名里带着谁的名字，那也只是这处房产归谁，不代表他人在这儿。
- 调查员已经在场景内了。不要再写进门、推门、踏入、拾级而上——那一步早就发生过了，重写一遍会让同一个动作在日志里连演两次。`;

const PROMPT_TEMPLATE_CLUE_REVELATION = `场景: {sceneName}
场景描述: {sceneDescription}
{worldContext}
检定的调查员（本段叙述唯一的行动者，所有动作的主语必须是这个人）: {investigator}
发现的线索: {clueName}
