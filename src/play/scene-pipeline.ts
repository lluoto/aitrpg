...
export async function processScene(ctx: SceneCtx): Promise<SceneConnection | null> {
  ...
  //
  // 实测（tools/_diag-phrasing.ts，1104 组 = 每个多出口场景 × 每个出口 ×
  // 17 种说法 × 原序/逆序两遍）：
  //   正例命中 726/726、反例（否定与「已经去过了」）144/144、
  //   歧义输入老实承认替选 234/234，且**原序与逆序结果完全一致**。
  // 这个数是判据先能区分对错之后才有意义的 —— 上一版跑出 100%，
  // 只是因为 12 条用例里 8 条都含完整地名。
  //
  // 剩下认不出的仍旧走这条路：后缀式别名（「医院」之于「霍姆斯医院」）、
  // 同义改写、代词、描述特征。**没听清就说出来**，别静默替他挑。
  if (picked.forced) {
