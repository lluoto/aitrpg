 ...
 * 这个不对称是留着的，不是漏改：本步已公布的实跑成绩就是拿现在这个解析器
 * 认下来的东西量出来的。放宽写法等于放宽「哪些回答算数」，那三个数会跟着动，
 * 而不重跑就没有新数去替换旧数 —— 于是公布的成绩变成一个没人量过的数字。
 * 要改的前提是先重跑一次、把三个数重新量出来，连着数一起改；只为了跟
 * classify-items 对称而对称，不值这个代价。
 *
 ...
 *
 * 开发·修 todo-51 重新测量（`extractKnownTitle` 放宽解析后，2026-09-02，
 * 真实 PDF + 真实 LLM ecnu-plus 实跑，`analysis/ingest/report.txt`）：
 * **命中 20 / 误报 3 / 漏报 0**——误报比此前公布的 4 少了 1 个，「关于艾米丽
 * 难产的事件」这次被模型判成 rule 而不是 scene。这个差异**分不清来源**：
 * 解析放宽本身不改变模型给出的分类结果（它只影响"键认不认得出来"），
 * 但这次重跑也是在放宽后才跑通的（放宽前 43 块解析不出来 42 个，整条链路
 * 走不到能对比这一步），无法排除是解析放宽附带的候选集变化，也无法排除
 * 是模型跨次运行的正常波动——如实两种可能都记下，不挑一个显得更确定的
 * 说法。剩下的 3 个真误报是「在小镇内询问路人」「旅店」「比较大的奇怪
 * 管道」，其中**旅店是个真地方**，只是基准没把它建成场景 —— 那不是体系
 * 缺格子，是基准的取舍，这条判断在新一轮里仍然成立。物品/条目分类/评分
 * 键左手边这几个数**没有变化**（覆盖 9/10、精确率 9/11、评分键 39/39），
 * 解析放宽只影响块分类这一层，不影响下游——见 `docs/notes/ingest.md` 这次
 * 重跑的完整记录。
 */
