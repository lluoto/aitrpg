...
export class GameSession {
  ...
  constructor(
    ...
    if (archetypeId) {
      try {
        ...
        // 创建角色卡档案（独立目录�?        const careerDir = `data/careers/${this.id}`;
  ...
  setPlayerHp(pid: string, value: number) {
    ...
    // 同步世界实体（若不存在则创建�?    let ent = this.world.getEntity(pid);
    if (!ent) {
  ...
  async act(input: string, actingCharacterName?: string): Promise<ActionResponse> {
    ...
    // NPC/同伴指令检�?    const inviteMatch = input.match(/^邀请\s+(.+)/);
    ...
    if (this.combatActive || /^(攻击|射击|挥砍|�?+攻击|�?+使用)/.test(input)) {
      ...
      if (enemies.length > 0) {
        ...
        // 检查战斗结�?        const aliveEnemies = Object.values(state.entities).filter(e => (e.type === "monster" || e.type === "npc") && e.hp > 0);
        if (aliveEnemies.length === 0) {
  ...
  private handleRest(messages: AgentMessage[], msg: (s: string) => number): boolean {
    // 获取角色 HP（优先从世界实体读取，回退�?activeCharacter�?    const state = this.world.getCurrentState();
    const playerEnt = state.entities["player"];
    ...
    // 技能成长检定（有标记时�?    this.lastRolls.push({ skill, roll, target: skillValue, success });`n    if (this.skillGrowthMarks && this.skillGrowthMarks.length > 0) {
    ...
    if (this._woundsTreated) {
      ...
      // 同步世界实体和角�?      if (playerEnt) { playerEnt.hp = newHp; this.world.upsertEntity(playerEnt); }
  ...
  private handleSkillCheck(intent: ActionIntent, messages: AgentMessage[], msg: (s: string) => number): boolean {
    ...
    // 记录技能标记（用于后续成长�?    this.lastRolls.push({ skill, roll, target: skillValue, success });`n    if (this.skillGrowthMarks && !this.skillGrowthMarks.includes(skill)) {
  ...
  private handleGenerateStory(messages: AgentMessage[], msg: (s: string) => number): boolean {
    ...
    // �?displayNames �?aliases 合并场景�?    for (const [id, name] of Object.entries(story.displayNames ?? {})) {
    ...
    // 设置当前场景为第一个场�?    if (story.scenes.length > 0) {
  ...
  private handleLoadModule(intent: ActionIntent, input: string, messages: AgentMessage[], msg: (s: string) => number): boolean {
    ...
    // 列出所有可用模�?    const allModules: Record<string, any> = {
  ...
  private handleRead(intent: ActionIntent, input: string, messages: AgentMessage[], msg: (s: string) => number): boolean {
    ...
    if (!tome) {
      // 非典籍物�?      msg(`你翻阅了�?{tomeName}」。`);
      this.lastNarrative = `你翻阅了�?{tomeName}」。`;
      return true;
    }

    // SAN 检�?    const result = this.sanity.sanityCheck(tome.sanCost);
    const passed = result.passed;
