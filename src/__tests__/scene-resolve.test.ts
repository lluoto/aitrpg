...
describe("精确匹配 — 认准了就不算替选", () => {
  test("展示名 / id", () => {
    expect(go("警察局")).toEqual({ sceneId: "警察局", forced: false, via: "display-name" });
    expect(go("警察局").sceneId).toBe("警察局");
  });
...
describe("包含匹配", () => {
  test("**正例**：地名在句中", () => {
    const r = go("警察局了解案情");
    expect(r.sceneId).toBe("警察局");
    expect(r.forced).toBe(false);
  });

  test("**正例**：反向包含（玩家只说了名字的一部分）", () => {
    expect(go("移动到谷仓").sceneId).toBe("谷仓形建筑");
  });
});
...
describe("提到 ≠ 要去 —— 这是改之前错的那三条", () => {
  test("**错误输入**：否定，且被否定的地名**更短**（改前靠长度蒙对）", () => {
    ...
    expect(r.sceneId).toBe("维森酒吧");
  });

  test("**错误输入**：否定，且被否定的地名**更长**（改前必错）", () => {
    // 原实现按「名字最长」挑，被否定的那个更长就一定选它。
    // 唯一「过了」的用例是靠长度运气 —— 换个更长的立刻现形。
    expect(go("别去艾德里安在镇子内的住宅，去报亭").sceneId).toBe("报亭");
    expect(go("不要去谷仓形建筑，去报亭").sceneId).toBe("报亭");
  });

  test("**错误输入**：已经去过了", () => {
    expect(go("警察局那边已经去过了，现在去报亭").sceneId).toBe("报亭");
  });

  test("**干扰输入**：句子里有「不」但不是修饰地名的 → 照常去", () => {
    expect(go("不管怎样先去警察局").sceneId).toBe("警察局");
  });

  test("**干扰输入**：所有候选都被否定 → 不该硬挑一个", () => {
    const r = go("别去警察局");
    expect(r.sceneId).not.toBe("警察局");
  });
});
...
describe("回归 — 修复过程中真的踩到的两个坑", () => {
  ...
  test("**坑 2**：过滤掉被否定的之后，反向包含那一支不能跟着失效", () => {
    ...
    // 正向包含没候选就整段跳过。
    expect(go("谷仓形建筑").sceneId).toBe("谷仓形建筑");
    expect(go("下水道").sceneId).toBe("下水道");
  });
});
