...
describe("字段口径", () => {
  ...
  test("非场景块上的 ▶ 条目同样计数 —— 那个数说的是全部块，不是场景块", () => {
    ...
    // 条目（实跑里 16 条）压根没进过任何计数，而字段注释写的是「不静默丢东西」。
    // 报出的数偏小且不留痕迹 —— 度量工具最不能有的就是这个失败方向。
    //
    // 前置块（title 为空）也一并算上：`sectionize.ts:102-103` 明确给
    // 「出现在任何标题之前的条目」留了位置，那些同样是 buildScenes 越过的。
    //
    // 这条测试同时钉住「全部块」这个口径：下游 toItemInputs 只收 scene 块上的条目
    // （`classify-items.ts:57-58`），所以这个数天生比下游见到的大。口径没改，
    // 是文案加了「含非场景块」把这件事说出来。
    const secs = [
