...
export class KPAgent {
  ...
  async describeScene(): Promise<string> {
    ...
    messages.push({
      role: "user",
      content:
        "请描述当前场景中玩家能看到、听到、感受到的一切。要先描述环境，再提示可能的行动方向。",
    });
    ...
    try {
      ...
      console.warn(`  ⚠ KP 场景描述 LLM 失败: ${err.message.slice(0, 60)}`);
  ...
  async narrateOutcome(
    ...
    messages.push({
      ...
      content: [
        ...
        `判定结果: ${outcome}`,
        "",
        "请以 KP 的口吻描述结果，推进一个叙事节拍。不要替 NPC 说话。",
      ].join("\n"),
    });
    ...
    try {
      ...
      console.warn(`  ⚠ KP 叙事 LLM 失败: ${err.message}`);
  ...
  async injectEvent(): Promise<string | null> {
    ...
    try {
      ...
      console.warn(`  ⚠ KP 事件注入 LLM 失败: ${err.message}`);
      return null;
    }
  }
